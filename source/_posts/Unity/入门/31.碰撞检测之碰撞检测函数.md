---
title: 31.碰撞检测之碰撞检测函数
date: 2026-01-31 09:18
categories:
  - 游戏引擎
  - Unity
  - 入门
...
### 知识点回顾
1. 如何让两个游戏物体之间产生碰撞（至少1个刚体和 两个碰撞器）
...
![图](images/Unity/入门/帧更新中函数执行的顺序.png)

### 知识点四 碰撞和触发器函数都可以写成虚函数 在子类去重写逻辑
一般会把想要重写的 碰撞和触发器函数 写成保护类型的 没有必要写成public 因为不会自己手动调用 都是Unity
通过反射帮助我们自动调用的

![图](images/Unity/入门/碰撞检测函数习题.png)