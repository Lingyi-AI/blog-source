...
![图](images/Unity/基础/场景异步加载习题.png)

``` CSharp
public class changjingyibujiazai
{
    private static changjingyibujiazai cjybjz = new changjingyibujiazai();
    public static changjingyibujiazai CJybjz 
    {
        get 
        {
            return cjybjz; 
    ...
    private changjingyibujiazai() { }

    public void Load(string str,UnityAction ac) 
    {
        AsyncOperation sm = SceneManager.LoadSceneAsync(str);
        sm.completed += (a) => 
        {
            ac();
        }; 
...
void Start()
{
    changjingyibujiazai.CJybjz.Load("text1", () => { print("测试"); });
}
```