...
#### 范围检测参数四全局设置
![图](images/Unity/基础/范围检测参数四全局设置.png)
...
void Update()
{
    ...
    {
        transform.Translate(Vector3.forward * Input.GetAxis("Vertical") * Time.deltaTime * 2);
    }
    if (Input.GetAxis("Horizontal") != 0) 
    {
        transform.rotation *= Quaternion.AngleAxis(1 * Input.GetAxis("Horizontal"), Vector3.up);
    }
    if (Input.GetKeyDown(KeyCode.J)) 
    {
        c = Physics.OverlapBox(transform.position + transform.forward, Vector3.one);
        print(transform.position + transform.forward);
        if (c.Length > 0) 
        {
            print(c[0].gameObject.name);
        }
    }
    if (Input.GetKeyDown(KeyCode.K))
    {
        c = Physics.OverlapCapsule(transform.position + transform.forward *5 + transform.up, transform.position + transform.forward * 5 + -transform.up, 1);
        print("点一" + (transform.position + transform.forward * 5 + transform.up));
        print("点二" + (transform.position + transform.forward * 5 + -transform.up));
        if (c.Length > 0)
        {
            print(c[0].gameObject.name);
        }
    ...
    {
        c = Physics.OverlapSphere(transform.position,10);
        if (c.Length > 0)
        {
            print(c[0].gameObject.name);
        }
