...
//由于新开启的线程无法直接改变Unity中的内容所以通过中间容器实现
void Update()
{
    if( queue.Count > 0 )
    {
...
void Update()
{
    if (Input.GetMouseButtonDown(0)) //点击鼠标开启协程
    {
        Coroutine c2 = StartCoroutine(dierti()); 
    }
}
IEnumerator MyCoroutine() //计时器
{
    while (true) 
    {
        print(i + "秒");
...
IEnumerator dierti() //创建100000个Cube
{
    for (int i = 0; i < 100000; i++)
    {
        GameObject g = GameObject.CreatePrimitive(PrimitiveType.Cube);
        g.transform.position = new Vector3(Random.Range(1, 1000), Random.Range(1, 1000), Random.Range(1, 1000));
        if (i % 1000 == 0) { yield return null; }   //等待下一帧执行
    }
}
```