...
void Update()
{
    if (Input.GetMouseButtonDown(0))
    {
        ...
        {
            GameObject g = Instantiate(obj1);
            GameObject g2 = Instantiate(obj2);
            g.transform.position = hit.point + hit.normal * 0.2f;
            g.transform .rotation = Quaternion.LookRotation(hit.normal);
            g2.transform.position = hit.point + hit.normal * 0.2f;
            g2.transform.rotation = Quaternion.LookRotation(hit.normal);
            Destroy(g, 5);
            Destroy(g2, 5);
        }
    }
...
void Update()
{
    if (Input.GetMouseButtonDown(0))
    {
        Ray r = Camera.main.ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;
        if (Physics.Raycast(r, out hit, 1000, 1 << LayerMask.NameToLayer("Player")))
        {
            obj3 = hit.collider.gameObject;
            print("选中物体" + hit.collider.gameObject.name);
        }
    }
    if (Input.GetMouseButton(0))
    {
        Ray r = Camera.main.ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;
        if (obj3 != null && Physics.Raycast(r, out hit, 1000, 1 << LayerMask.NameToLayer("Default")))
        {
            obj3.transform.position = hit.point;
            print("拖动物体" + obj3.name);
        }
    }
